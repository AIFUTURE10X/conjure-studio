...
- **Migrations**: apply with `node scripts/run-sql.cjs scripts/<file>.sql` (reads `.env.local`). 008/009/010, 017 (`provider_usage`, the per-call cost ledger) and 018 (`favorites.source_url` + `content_hash`) are applied to the production Neon DB. Number a new migration above the highest file in `scripts/`, not the highest one you remember — 016 was claimed twice.
